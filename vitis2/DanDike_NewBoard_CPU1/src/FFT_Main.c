...
void memoryToEmxArray(uint32_t base_addr, int Sample_Points, int Adc_Sampling, int channel, emxArray_real_T *emxArray) {
    ...
    for (int k = 0; k < Adc_Sampling; k++) {
        for (int i = 0; i < Sample_Points; i++) {
            uint32_t addr = base_addr + channel * Sample_Points * 4 + k * 8 * Sample_Points * 4 + i * 4;
...
void FFT_Calculate(int chanel){
	...
	//初始化结构体
	init_emxArray_real_T(&var1, 1, sample_points * AD_SAMP_CYCLE_NUMBER);

	// 将内存数据传输到 emxArray_real_T
	memoryToEmxArray(Share_addr, sample_points, AD_SAMP_CYCLE_NUMBER, chanel, &var1);//通道3做FFT
