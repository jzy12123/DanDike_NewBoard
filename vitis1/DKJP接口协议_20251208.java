...
(
	...
	(
		...
		上行:
		{
			...
			"Result":"Success",
			"Data": {
				"CmdList": {			//该节点下，带*的表示Svr端扩展的功能，不带*的表示Arm端原生支持功能；通信时报文中应删除*标识；
				...
				"StructList": [1,100,101...]   				//Arm端支持主动上送的结构体标识列表，PC端不用关心此属性； 
			}							 
		}
	)  
	...
	(
		...
		上行:
		{
			...
			"Data": {										//下列项和仪器功能相关,均为可选相
				...
				"WaveReplayPathArm" : "/tmp/waveReplay",    //Arm端采用ftp上传波形回放的原始文件时，此节点告知上传路径。不支持该模式的不出现该节点。 
				"WaveReplayPathSvr" : "/usr/waveReplay",  		//(ARM端无此节点)Svr端告诉UI用户，ftp上传波形回放的原始文件的上传路径。  
	...
	(
		下行:
		{
			...
			"Data": {      //下行中，没有出现的属性表示值不变！
				...
				"AC": {
					"ConstantMode":"Total/Fund",		//Total：总有效值恒定（缺省）；Fund：基波恒定
					"HarmCalcMethod": "Subgroup"/"Group",	//谐波计算方法   子组/组
					"HarmPhaseRef":  "FundUa"/"FundChn",   //谐波相位基准 
...
(
	...
	(
		...
		(
				...
				//回复 Reply.WaveRecord 表示接受录波指令，开始执行，可能立即启动，也可能在等待时刻或者开入条件中，等录波真正启动时由 Report.WaveRecordStart 再次上报。
				//录波结束时由 TaskEvent.WaveRecord（先报） 和 Report.WaveRecordComplete（后报） 两次上报。
		)
		...
		(
			下行:
			{
				...
				"Data": {
						...
						"StartTime":"2024-01-01 08:00:00.123",			//仅 StartMode=1 时有效,指明启动时刻。	 //推荐使用整分
...
(	
	...
	(
		...
		(
			...
			● 111=TWData, 行波测试仪数据             
				(
					struct TWData   //                  8*8*2 => 128 Byte
					{
						double f1, ir1, ia1, ib1, ic1, phia1, phib1, phic1;   			//第1路的频率、电流档位，三相电流幅值和相位
						double f2, ir2, ia2, ib2, ic2, phia2, phib2, phic2;   			//第2路的频率、电流档位，三相电流幅值和相位 
					};			
				)
		)
		【3】直流源类型(300~399)
