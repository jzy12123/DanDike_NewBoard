...
(
	20250809: {
		...
		2、 修改 20.3【结构体类型标识】中的 100 标识：DevState，调换了bACRunning和bClosedLoop的位置，同时增加bHarmRunning状态；
	...
	20250610: {
		...
		3、 新增 1.7UI端修改服务端的零位处理（ARM不支持） 指令 SetServerZeroDrift，用于UI端修改服务端的零位处理参数；
	...
	20250527: {
		...
		-10、 新增 12.0校准系数恢复缺省值  指令 RestoreCalibrateDefault；
...
(   
	(
		本协议适用于采用核心板(CoreBoard,简称C端; 或者AMP的M核,CPU1)+工控机(IPC,简称I端; 或者AMP的A核,CPU0)架构的装置与上位机(PC,简称P端)各模块之间的通讯。 
		协议采用TCP长连接。报文格式采用标准JSON,每帧报文发送时需在首尾添加“|”字符,即“|{...}|”形式。
		
		I端与P端通讯时,全部采用TCP连接。I端为服务端,P端为客户端;
		
		I端与C端通讯时,除周期上送的Report部分外,其它指令和I/P端通讯完全一致。I端为服务端,P端为客户端;	
		C端的Report部分采用UDP或内存共享方式,发送结构体至I端(单向传输),具体结构体格式详见后文。
	)
	...
	0.3.4上行,Report(基础数据上报)
		...
		下行报文参数如果不合理,服务端应回错误原因;
		协议中模拟量通道摒弃ABCX等相别描述方式,采用 1,2,3,4 序号对应表示;
		 
		服务端接收下行报文后先进行报文有效性判断,如果JSON语法正确,则C端必须进行回应;
	...
	(	1. 文档中涉及的U/I/P/Q/F等电参量的单位,如未特殊说明的,均采用国标单位,即:
		...
		6. 双方处理数据时,如果涉及比较大小时,应采用浮点数的模糊比较模式。如量程切换中常见的 57.7/57.74/57.735 场景。
		7. 协议采用JSON格式,应注意单词是区分大小写的!同时,本协议中的单词除个别约定俗成外,均采用大驼峰命名形式。
	
		8. 建议RK3568地址分配
				eth0: C端和I端间内部通讯用,配置为: 		192.168.98.241;
				eth1: I端对外提供通讯协议或者连接被试终端用,IP可自行设置;
				C端(核心板)TCPS的设置为: 			192.168.98.240:19980;  
				C端(核心板)UDP发送目标设置为: 	192.168.98.241:19981;
				I端(嵌入式)TCPS对外提供通讯服务端口为: 19982;  
...
(
	...
	(
		...
		上行:
		{
			...
			"Result":"Success",
			"Data": {
				"CmdList":	["GetFunCodeList","GetDevBaseInfo","GetDevState","SetServerReportEnable",
							 "SetDCS","SetDCM","SetACS","SetACM","SetHarm","SetInterHarm","SetDO","SetDI","SetPulseOut", 
							 "StopDC",
							 "SetACStatus",
							 "SetHarmStatus",
							 "ClearInterHarm","","",""],
				"ReportList":		["ProtectEvent","DISOE","BaseDataAC","BaseDataDC","BaseDataIO","HarmData","InterHarmData"],			//常规数据上报
				"TaskEventList":	["SetSysTimeSyncMode","SetProgress","SetTaskClockTest","SetTaskEnergyTest",""]												//事务数据上报
			}							 
		}
	)
		...
		上行:
		{
			...
			"Data": {										//下列项和仪器功能相关,均为可选相
				...
				"AC":{
					...
					"MaxHarmNumber": 63,					//最大谐波次数   【间谐波一致吗?】
	...
	1.5UI端修改服务端的数据主动上送使能及间隔设置（ARM不支持） 
	(
		下行:
		{
			"FunType": "Cmd",	 
...
(
	...
	(
		...
		{
			"FunType": "Report",	 
			"FunCode": "ProtectedEvent",	//所有故障仅表明刚发生了该故障,发生故障后装置均会停止工作(如:降源、关闭等)。 //ARM需考虑同时发生的故障用一帧上送
			"Data": {
				"UShort":  [	//电压短路,数组中元素为通道号,可选
	...
	(
		上行:
		{
			...
			"FunCode": "RecStart",
			"Data": {
				"RecFile": "\...\xxx.dat",						//录波文件所在路径及文件名，约定用 .dat 作为文件扩展名；
																			//文件名缺省按： YYYYMMDDHHNNSS.dat / YYYYMMDDHHNNSS.cfg 形式命名，其时刻为粗略到秒的录波开始时刻，更精确的时刻在cfg文件中描述。
				"RecFrom":"StateSequence"/"WaveformReplay"/"DI", 		//录波来源，DI指专用于启动录波的开入节点从0变位1的信号？
				"RecDuration": 30000,						//预估录波时长，单位ms，无法确定时用-1表示？
				"ErrInfo": "InsufficientStorage"			//仅启动失败时包含该属性。如：磁盘空间不足。
			}
		}
	)
	2.2.2录波完成
	(
		上行:
		{
			...
			"Data": {
				"RecFile": "\...\xxx.dat",						//录波文件所在路径及文件名，约定用 .dat 作为文件扩展名；
																			//文件名缺省按： YYYYMMDDHHNNSS.dat / YYYYMMDDHHNNSS.cfg 形式命名，其时刻为粗略到秒的录波开始时刻，更精确的时刻在cfg文件中描述。
				"RecFrom":"StateSequence"/"WaveformReplay"/"DI" 		//录波来源
	...
	(
		上行:
		{
			...
			"Data": {
				...
				"Mode":"S/M",                		//源当前工作模式
				"ACRunning": 0/1/2,					//0表示停止、2表示暂停：无"Chns/Lines"节点；1表示运行：有"Chns/Lines"节点；	
	...
	(
		上行:
		{
			...
			"Data": {		//按线路顺序列出每线路包含通道的实时基本数据信息
				"ACRunning": 0/1,					//0表示未运行,无"Chns"节点;1表示运行,有"Chns"节点	
	...
	2.8间谐波周期上报(ARM不支持)
	(
		格式同谐波周期上报,"FunCode" 为 "InterHarmData"
	)
...
(
	...
	(
		下行:
		{
			"FunType": "Cmd",	 
			"FunCode": "SetACM",					 
			"Data": [		//按线路顺序列出需要设置的量程信息(可只列需要改变的项)
						{"Line":1, "Chn":1, "UR": 57.7, "IR": 5},	
						{"Line":1, "Chn":2, "UR": 57.7, "IR": 5},
						{"Line":1, "Chn":3, "UR": 57.7, "IR": 5},
						{"Line":1, "Chn":4, "UR": 57.7, "IR": 5},
	...
	3.3.3谐波输出设置 【注意：指令中谐波U、I均用含量（百分数）表示】
	(
		下行:
		{
			"FunType": "Cmd",	 
			"FunCode": "SetHarm",				 
			"Data": [ 		//按线路顺序列出需要设置的输出数据(可只列需要改变的项)
						//方式一  [Line, Chn, HN, U, PhU, I, PhI]
						[1, 1, 2, 0.1, 0, 0.1, 0],	 	//不设置的次数可以不包含
						...
						[1, 1, 63, 0.1, 0, 0.1, 0],
						...
						[2, 4, 2, 0.1, 0, 0.1, 0],	 	 
	...
	3.3.4谐波输出 暂停 / 运行(恢复上次参数) / 停止(清空参数)     
	(
		下行:
	...
	//旧版， 需将原来的 ClearHarm 代码修改为 SetHarmStatus.Stop。
	(
		下行:
		{
			"FunType": "Cmd",	 
	...
	3.3.6清空间谐波
	(
		//同谐波设置,仅 FunCode 为 "SetInterHarm"/"ClearInterHarm", 谐波次数为 0.1,0.2,0.5的整数倍?
	)

	3.3.7交流表或源 的 暂停/恢复/关闭(同时清空所有基波、谐波、间谐波输出)
	(
		下行:
		//Stop: 不管是 Pause 还是 Run 状态均有效，清除设置的所有基波、谐波、间谐波输出参数并停止交流所有输出；	
		//下位机一旦接收到 SetACS 指令后，自动转入 Run 运行状态； 
		{
			"FunType": "Cmd",	 
			"FunCode": "SetACStatus",				 
			"Data": "Pause/Run/Stop"				
	...
	//旧版， 需将原来的 StopAC 代码修改为 SetACStatus.Stop。
	(
		下行:
		{
			"FunType": "Cmd",	 
			"FunCode": "StopAC"				//StopAC: 不管是PauseAC还是ContinueAC状态均有效，清除设置的所有基波、谐波、间谐波输出参数并停止交流所有输出；	 	 							 
		}
		上行:
		{
			"FunType": "Reply",	 
...
(
	...
	(
		...
		上行:            
		{
			...
			"Result":"Success/Failure",		
			"Data": { 
						"Tasks":[ "SetTaskClockTest","SetTaskGradualChange", ... ]		//枚举正在运行中的任务列表
			}		 
		}
	)
	...
	(
		下行:
		{
			...
			"Data": {
				...
				"FreqDivFactor": 1,							//分频系数: Frequency Division Factor
		...
		上行:(过程中及结束时上报)  //推荐有新的误差时上报
		{
			...
			"Data": [			//元素个数对应 GetDevBaseInfo 中 EnergyPulse.ChnCountIn  的值
					{   "Chn": 1,								//通道1
	...
	(
		下行:
		{
			...
			"Data": {
						...
						"Round":10, 			//Round: 测试圈数;  
						"TestTimes":5			//TestTimes: 目标测试次数,缺省为1 ，范围：1~99. 						【建议TestCount修改为TestTimes】
					}							 
		}                 
		上行:(仅应答回一次)            
		{
		...
		上行:(过程中及结束时上报)  //推荐有新的误差时上报
		{
			...
			//新版：采用数组，支持多通道
			"Data": [			//元素个数对应 GetDevBaseInfo 中 ClockPulse.ChnCountIn 的值
					{   "Chn": 1,								//通道1
						"Round":10, 						//Round:  当前正在测试圈数;  								
						"TestedTimes":2,					//TestedTimes: 当前已经完成测试次数;		
						"Errs": [ 0.02, 0.04, ... ]		//TestedTimes 顺序对应的误差列表,元素个数与TestedTimes一致，单位：%		
	...
	(
		...
		{//旧版，弃用
			...
			"Data": {					
					...
					"RecFile":"waverec",  	//波形文件名,假如装置只支持存放一个文件,则文件名为缺省的"waverec"。							
					...
					"States":[
							{		//step1
								...
								"TrigDI":[							//开入触发时提前跳转,此值给出触发条件,
											{"Chn":1, "val":0/1},		
											{"Chn":2, "val":0/1},
								],								
								//"TrigLogicAnd":true/false,      //false为或;缺省为true表示与逻辑。
								"TrigLogic": -1/0/1, 				  //-1=禁用（缺省），0=或；1=与；								
								"JumpTo":0,						//跳转目标: 0=下一步(缺省值,条件到或延时到均跳下一步);-1=开入到跳结束(条件不满足而延时到跳下一步);-2=延时到跳结束(如果延时到且开入条件不满足跳下一步);
			...
			"Data": {					
					...
					"States":[
							{		//step1
								...
								//==============
								// 所有 跳转目标步号 的定义及取值范围为：-2/-1/0/(1...n)    
								// -2=直接结束；
								// -1=跳到最后一步（执行最后一步）; 
								// 0=下一步(缺省值);  
								// >0=跳到指定步(状态序列步号从1计，范围：1...n,  其中n为状态序列的总步数) 原则上不允许出现n大于总步数的情况，如果出现，下位机当做-1处理;
								//==============
								"TimeoutTargetStep":  -1/0/(1...n)    			//超时跳转目标 (缺省值为0);
								"DIJumpRules": [             //下位机在刚进入该步或开入变位时需要检测是否满足规则条件，取第一个满足规则的进行跳转。
										{
											"ConditionMask": 0x03 //0b00000011		// 与逻辑，Bit0 (开入1) 和 Bit1 (开入2) 需要检查，其他位忽略
											"ExpectedState":  0x01 //0b00000001       // 与逻辑，期望 Bit0 (开入1)=1(ON), Bit1 (开入2)=0(OFF)
											"TargetStep":  4								//满足条件时跳转目标 (缺省值为0);
										},
										{
											"ConditionMask": 0x04 //0b00000100		// 与逻辑，Bit2 (开入3) 需要检查，其他位忽略
											"ExpectedState":  0x04 //0b00000100       // 与逻辑，期望 Bit2 (开入3)=1(ON)
											"TargetStep":  6								//满足条件时跳转目标 (缺省值为0);
										},
										...
								]								
								//==============
	...
	(
		...
		(
				...
				//回放时PC端只需提供dat文件(cfg参数由WaveformReplay指令提供),录波时装置需保存cfg和dat文件。
				//录波结束主动通过Report上报RecComplete。			
		...
		(
			...
			{
				"FunType": "Cmd",	 
				"FunCode": "WaveformReplay",					 
				"Data": {
						"WaveFile":"waveplay",  		//波形文件名,假如装置只支持存放一个文件,则文件名为缺省的"waveplay"。
						"WaveSamp": 12800			//波形数据的采样率(应与数据文件一致);(取值上限:51200)
						"Freq": 50,							//额定频率
						"StartMode":"",					//启动模式:0=立即启动;1=定时启动; 		
						"StartTime":"2024-01-01 08:00:00.123",			//仅定时启动模式时有效,指明启动时刻。	
						...
						//"TrigLogicAnd":true/false,      //仅“RepeatCountPrev”=-1时有效;缺省false为或;true为与	
						"TrigLogic": -1/0/1, 				  //-1=禁用（缺省），0=或；1=与；
			...
			上行:(仅应答回一次)       
			{
				...
				"Result":"Success/Failure",		
				"Data": {
					"ErrInfo":"WaveFileNotExist"/"InsufficientStorage"						//仅Failure时存在，错误信息描述,如:波形文件不存在, 磁盘空间不足等				
				}		 
			}
			上行:(结束时上报)
			{
...
(
	...
	(
		...
		(
			上行:
			{
				...
				"Data": {							
					...
					"ACRunning": 0/1,					//表源运行状态:0表示未运行,1表示运行 
					"vals":{
		...
		10.1.6 (DK51D)UI端下发与终端的联机命令,服务端执行联机,并返回结果 
		(
			下行:
			{
				"FunType": "Cmd",	 
		...
		10.1.7 (DK51D)UI端下发测试任务,服务端执行检测,并返回结果——(测试结束源输出保持)
		(
			下行:
			{
				"FunType": "Cmd",	 
		...
		10.1.8 (DK51D)UI端下发停止测试任务(停止后源输出保持)
		(
		下行:
			{
				"FunType": "Cmd",	 
...
12.校准 
(
	12.0校准系数恢复初始值  //谨慎使用，会影响装置准确度        
	(
		下行:
		{
			"FunType": "Cmd",	   
			"FunCode": "RestoreCalibrateDefault",					 
			//"Data": ["AC","DCS","DCM"]			//需要恢复的项目列表	
			"Data": {
...
20.C端与I端间的UDP或内存共享传输结构体
(	
    //所有Report指令仅用于I端和P端之间;C端和I端之间不采用,改为UDP结构体传输， 开机后C端默认所有Report使能为true开启状态。 
	//适当考虑冗余(保留字)!
	//采用C->I单向传输;
	...
	20.2【说明】
	{
		1、 开机缺省所有类型均主动上送，I端可通过指令"SetReportEnable"修改是否上送标志。
	...
	(
		...
		(
			/* ● 1=故障等保护信息 		//弃用,改用JSON上传!
				(
					按U、I的通道顺序列出故障状态,每个通道1字节,1=有故障,0=无故障;0xFF=保留(无此通道,仅用于字节对齐时占位)
					...
					...		
					//过热:				//1=有故障,0=无故障				
					【特殊说明】:本结构具体组成由相应的装置型号定义。但应保证总字节数为8的倍数,不足时应用0xFF凑齐。
				)
			*/
			...
			● 100=DevState, 装置状态 		 
				(
					struct DevState    	//16字节    //所有Line和Chn均同步使用下列状态；
					{
						...
						byte     bClosedLoop;          1字节		//0=开环状态;1=闭环状态		
						byte     bACRunning;           1字节		//0=停止状态;1=运行状态；2=暂停状态   						
						byte     bHarmRunning;       1字节      //0=停止状态;1=运行状态；2=暂停状态。	注：bACRunning 和 bHarmRunning 相互独立，互不影响。
						byte     Reserved4;               1字节
						byte     Reserved5;               1字节
						byte     Reserved6;               1字节
						byte     Reserved7;               1字节					
		...
		【2】交流量类型(101~299)
		(
			● 101=BaseDataAC,交流源/交流表基础数据			//源模式且源停止状态时,应置结构体长度=0, 非0表示源输出中。
				(
			...
			● 102=HarmData,谐波数据【数组中元素0表示直流幅值；1表示基波幅值；从2开始为谐波，谐波U、I用含量（百分数）表示，P、Q用幅值表示】
				(
					struct Harm    //交流通道谐波                 HarmNumberMax*6*8+16  => HarmNumberMax为32时=1552
					{
						...
						double totalP=0;        //当前通道的总有功
						double totalQ=0;
					};
					
					struct LineHarm   //交流线路——谐波              ChnsAC*(HarmNumberMax+1)*6*8+16  => 3*32=3*1552=4656
					{
						Harm   harm[ChnsAC];  		//harm[ChnsAC]
					}
				)
				
			● 103=InterHarmData,间谐波数据   
			...
			● 110=VMData, 电压监测仪数据               //除了整定误差用秒数据,其它试验全部用分钟数据。
				(
					...
					{
						byte     bACMeterMode;     1字节		//0=交流源状态;1=交流表状态
						byte     bACRunning;           1字节		//0=停止状态;1=运行状态
	...
	(	
		...
		//{
		//	Harm   interharm[ChnsAC];  		//interharm[ChnsAC]
		//}
		...
		struct Data
		{
			...
			//LineInternHarm   interharm[LinesAC];   
			//        ChnDC  dcs[ChnsDCS];
			//        ChnDC  dcm[ChnsDCM];
			//        ChnIO  bi[ChnsBI];		 
			//        ChnIO  bo[ChnsBO];		 
