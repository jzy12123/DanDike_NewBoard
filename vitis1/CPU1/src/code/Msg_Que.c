...
ssize_t MsgQue_write(const char *buf, size_t cnt)
{
    ...
    memset((void *)data_addr, 0, MAX_DATA_LEN);                                    // 在写数据之前先清空当前数据区，防止上次残留数据影响本次写入的数据。
    Xil_DCacheFlushRange((INTPTR)Rtos_WriteMemorySpace, sizeof(struct msg_queue)); // 清空缓存，防止上次残留数据影响本次写入的数据。
