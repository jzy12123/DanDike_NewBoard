...
volatile int g_Debug_UartIrqCnt = 0; // 调试计数器
// 自定义中断处理函数
void GpsUartRecvHandler(void *CallBackRef)
{
	...
	XUartLite *UartInstPtr = (XUartLite *)CallBackRef;
	u32 StatusRegister;
	u8 ReadByte;

	// 1. 读取状态寄存器
	StatusRegister = XUartLite_GetStatusReg(UartInstPtr->RegBaseAddress);

	// 2. 检查是否有数据 (RX FIFO Valid Data)
	// 只要 FIFO 不为空，就一直读，直到读空为止
	while (!XUartLite_IsReceiveEmpty(UartInstPtr->RegBaseAddress))
	{
		// 直接从硬件 FIFO 读取一个字节
		ReadByte = XUartLite_ReadReg(UartInstPtr->RegBaseAddress, XUL_RX_FIFO_OFFSET);

		// 3. 存入你的缓冲区
		if (GPS_Ctrl_State.uart_cont < (1000 - 1))
		{
			UART_RX_BUF[GPS_Ctrl_State.uart_cont] = ReadByte;
...
//	 }
	...
	// 如果你在 BareMetal_Intc_Handler 里调用了 XIntc_InterruptHandler，那就没问题。
}
...
void GpsPPSHandler(void *CallBackRef)
{
	...
	// 【探针 2】直接读取 UART 硬件寄存器的值 (偏移量 0x08)
	u32 HardwareStatus = XUartLite_ReadReg(GpsUartLiteInst.RegBaseAddress, XUL_STATUS_REG_OFFSET);

	xil_printf("PPS Debug: IRQ_Cnt=%d, UART_Status=0x%02X, Soft_Cnt=%d\r\n",
			   g_Debug_UartIrqCnt, HardwareStatus, GPS_Ctrl_State.uart_cont);
