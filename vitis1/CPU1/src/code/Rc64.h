...
#define I2C_DEVICE_ID \
    XPAR_RTC_EEPROM_AXI_IIC_0_DEVICE_ID // AXI IIC 设备ID
                                        // (低层级驱动可能不直接使用)
...
// #define IIC_TIMEOUT_COUNT 100000           //
// 等待总线空闲或操作完成的超时计数值
#define EEPROM_WRITE_ACK_POLL_RETRIES \
    500 // EEPROM 写操作后轮询 ACK 的最大重试次数
#define EEPROM_WRITE_ACK_POLL_DELAY_US \
    500                              // EEPROM 写操作后轮询 ACK 的间隔 (微秒)
#define EEPROM_ADDR_SEND_RETRIES 100 // EEPROM 读操作时发送地址的最大重试次数
#define EEPROM_ADDR_SEND_DELAY_US \
    100 // EEPROM 读操作时发送地址的重试间隔 (微秒)
...
#define ARRAY_BYTES        \
    (ARRAY_ELEMENT_COUNT * \
     DOUBLE_SIZE) // 每个校准数组的总字节数 (8*3*8 = 192 bytes)

// --- EEPROM Address Mapping ---
// 定义校准数据在 EEPROM 中的存储起始地址
#define EEPROM_ADDR_DA_CORRECT_100 (0) // 第一个数组的起始地址
#define EEPROM_ADDR_DA_CORRECT_20 \
    (EEPROM_ADDR_DA_CORRECT_100 + ARRAY_BYTES) // 第二个数组的起始地址
#define EEPROM_ADDR_DA_CORRECTPHASE_100 \
    (EEPROM_ADDR_DA_CORRECT_20 + ARRAY_BYTES) // 第三个数组的起始地址
#define EEPROM_ADDR_AD_CORRECT \
    (EEPROM_ADDR_DA_CORRECTPHASE_100 + ARRAY_BYTES) // 第四个数组的起始地址
#define EEPROM_TOTAL_CALIB_BYTES \
    (EEPROM_ADDR_AD_CORRECT + ARRAY_BYTES) // 校准数据占用的总字节数

// --- CRC Definitions ---
// CRC-16/CCITT 校验值存储在所有校准数据之后
#define EEPROM_ADDR_CALIB_CRC \
    (EEPROM_TOTAL_CALIB_BYTES) // CRC16 在 EEPROM 中的存储地址
#define CALIB_CRC_INIT 0xFFFF  // CRC-16/CCITT 初始值
