...
const char ARM_Ver_Full[] = "[Ver]=V1.260515.0951";
...
send_reply:;
    ...
    // 如果是手动对时也要回复任务成功
    if (mode_to_start == SYNC_MODE_MANUAL)
    {
        g_TimeSyncManager.current_mode = mode_to_start;
...
Struct_WaveRecordTask g_WaveRecordTask;
// 辅助：比较时间字符串 (Mock implementation or use standard libs)
// 返回: -1=expired, 0=ok/future
static int Check_StartTime_Validity(const char *targetTimeStr)
{
    // 简单实现：获取当前时间对比
    In_CurrTime curr;
    ...
    if (t_mon < curr.curr_month)
        return -1;
    if (t_mon > curr.curr_month)
        return 0;

    if (t_day < curr.curr_day)
        return -1;
    if (t_day > curr.curr_day)
        return 0;

    if (t_hour < curr.curr_hour)
        return -1;
    if (t_hour > curr.curr_hour)
        return 0;

    if (t_min < curr.curr_minute)
        return -1;
    if (t_min > curr.curr_minute)
        return 0;

    if (t_sec < curr.curr_second)
        return -1;

    return 0; // Future or Equal
}

void handle_SetTaskWaveRecord(cJSON *data)
{
    // 1. 解析参数
    cJSON *item;

    item = cJSON_GetObjectItem(data, "StartMode");
    g_WaveRecordTask.StartMode = item ? item->valueint : 0;

    item = cJSON_GetObjectItem(data, "StartTime");
    if (item)
        strncpy(g_WaveRecordTask.StartTime, item->valuestring, 31);

    // 解析 DI 触发条件
    g_WaveRecordTask.TrigDICount = 0;
    cJSON *arr = cJSON_GetObjectItem(data, "TrigDI");
    if (arr)
    {
        int count = cJSON_GetArraySize(arr);
        if (count > 16)
            count = 16;
        g_WaveRecordTask.TrigDICount = count;
        for (int i = 0; i < count; i++)
        {
            cJSON *elem = cJSON_GetArrayItem(arr, i);
            cJSON *chn = cJSON_GetObjectItem(elem, "Chn");
            cJSON *val = cJSON_GetObjectItem(elem, "val");
            if (chn && val)
            {
                g_WaveRecordTask.TrigDIs[i].Chn = chn->valueint;
                g_WaveRecordTask.TrigDIs[i].Val = val->valueint;
            }
        }
    }

    item = cJSON_GetObjectItem(data, "TrigLogic");
    g_WaveRecordTask.TrigLogic = 0; // Default OR
    if (item && item->valuestring)
    {
        if (strcasecmp(item->valuestring, "And") == 0)
            g_WaveRecordTask.TrigLogic = 1;
    }

    item = cJSON_GetObjectItem(data, "RecMS");
    g_WaveRecordTask.RecMS = item ? item->valueint : 5000;

    item = cJSON_GetObjectItem(data, "RecSamp");
    g_WaveRecordTask.RecSamp = item ? item->valueint : 51200; // 实际使用 FS_RATE
    ...
    {
        if (Check_StartTime_Validity(g_WaveRecordTask.StartTime) < 0)
        {
            errInfo = "StartTimeExpired";
        }
    }
    ...
    cJSON *reply = cJSON_CreateObject();
    cJSON_AddStringToObject(reply, "FunType", "Reply");
    cJSON_AddStringToObject(reply, "FunCode", "SetTaskWaveRecord");

    if (errInfo != NULL)
    {
        cJSON_AddStringToObject(reply, "Result", "Failure");
        cJSON *errData = cJSON_CreateObject();
        cJSON_AddStringToObject(errData, "ErrInfo", errInfo);
        cJSON_AddItemToObject(reply, "Data", errData);

        // 校验失败，不启动任务
        g_WaveRecordTask.State = 0;
    }
    else
    {
        cJSON_AddStringToObject(reply, "Result", "Success");
        cJSON_AddItemToObject(reply, "Data", cJSON_CreateObject());

        // 4. 任务初始化 (修改后)
        if (g_WaveRecordTask.StartMode == 0)
        {
            // [Mode 0] 立即启动
            g_WaveRecordTask.State = 3; // Recording
            WaveRecord_Start(g_WaveRecordTask.RecMS, "SetTaskWaveRecord");
        }
        else if (g_WaveRecordTask.StartMode == 1)
        {
            // [Mode 1] 定时启动 -> 设置软时钟闹钟
            // 注意：EnableAlarm 会写入寄存器，随后只需等待中断
