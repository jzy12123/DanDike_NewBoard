...
{
	// --- 下发参数 ---
	int StartMode;		// 0=立即, 1=定时, 2=DI触发
	char StartTime[32]; // 定时启动时间字符串

	int TrigDICount;			   // 触发DI数量
	Struct_WaveTrigDI TrigDIs[16]; // 最大支持16个条件
	int TrigLogic;				   // 0=OR, 1=AND (解析 "Or"/"And")

	int RecMS;	 // 录波时长
	int RecSamp; // 采样率 (目前固定)

	// --- 运行时状态 ---
	int State; // 0=Idle, 1=WaitTime, 2=WaitDI, 3=Recording
	// 任务是否刚刚完成录波（用于发送 TaskEvent）
	bool IsDonePendingReport;

} Struct_WaveRecordTask;

extern Struct_WaveRecordTask g_WaveRecordTask;
/******************************************************************************************************
 * 函数申明
 ******************************************************************************************************/
