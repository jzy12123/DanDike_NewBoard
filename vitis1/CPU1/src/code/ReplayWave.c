...
static int ReplayWave_ParseCfg(const char *cfgStr)
{
    ...
    while (lineStart && *lineStart)
    {
        ...
        else if (lineNum == 2)
        {
            ...
            if (cfg->analogCount > REPLAY_MAX_FILE_CHANNELS)
    ...
    /* 验证必要字段 */
    if (cfg->analogCount == 0 || cfg->totalSamples == 0)
    {
        xil_printf("ReplayWave: ParseCfg FAIL: analogCount=%d, totalSamples=%u\r\n",
                   cfg->analogCount, (unsigned int)cfg->totalSamples);
        return -1;
    }
...
const char *ReplayWave_HandleParas(cJSON *data)
{
    ...
    if (!cfgItem || !cfgItem->valuestring)
    {
        return "CfgParseError";
    }
    int ret = ReplayWave_ParseCfg(cfgItem->valuestring);
    if (ret != 0)
    {
        return "CfgParseError";
    }
