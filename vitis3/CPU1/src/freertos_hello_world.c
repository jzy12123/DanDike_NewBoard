...
//	sleep(17);				//必须要有等待linux启动
	...
	float fundamental_frequency = 0.0f; // 基频
	float fundamental_value = 0.0f;     // 基频值
	float harmonic_info[32][3] = {0};
    AnalyzeWaveform();
    // 打印结果
	printf("Fundamental Frequency: %.2f Hz\n", fundamental_frequency);
	printf("Fundamental Value: %.2f\n", fundamental_value);

	printf("Harmonic Information:\n");
	for (int i = 0; i < MAX_HARMONICS; i++) {
	 printf("harmonic %d: ", i + 1);
	 for (int j = 0; j < 3; j++) {
		 printf("%.2f ", harmonic_info[i][j]);
	 }
	 printf("\n");
	}
