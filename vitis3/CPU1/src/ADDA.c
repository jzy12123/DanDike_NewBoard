...
//波形修改参数
float Phase_shift[8] 	= 	{0 , 120 , 240 , 0 , 60 , 180 , 300 , 0};		//8路波形相位偏移 单位度
...
void adc_start(){
	...
	if(sample_points == 256) {
		Xil_Out32(adc_whole_base_addr + 8, 0x100);					//采样点：sample_points写1024
		Xil_Out32(adc_whole_base_addr + 4, 0x1E84);					//7812，对应采样频率50*256
	}

	else if(sample_points == 1024) {
		Xil_Out32(adc_whole_base_addr + 8, 0x400);					//采样点：sample_points写1024
		Xil_Out32(adc_whole_base_addr + 4, 0x7A1);					//1953，对应采样频率50*1024
	}

	else if(sample_points == 2048) {
		Xil_Out32(adc_whole_base_addr + 8,  0x800); 				//sample_points写2048
		Xil_Out32(adc_whole_base_addr + 4,  0x3D0); 				//976，对应采样频率50*2048
	}

	else{
		printf("CPU1: adc parameter error\n ");
	}
...
//DMA RX中断处理函数 adc
 ...
 void adcS_intr_handler(void *callback) {
	...
	//继续开启新一次的ADC
	if(ADC_ChannelEnable){//ADC_ChannelEnable不为0，则开启ADC
		adc_start();
	}
		...
		u16 rx_intr_id, u16 adcS_id, u16 tx_intr_id, u16 overflow_id, u16 underflow_id, u16 switch_id,u16 amplifier_id,u16 SoftIntrCpu1_id,u16 Timer_id)
{
 ...
 void Adc_Data_processing() {
	...
	for(int j = 0;j < CHANNL_MAX;j ++){					//8个通道
		for(int i = 0; i < sample_points; i ++){
			//共享基地址 + 8通道偏移地址 + AD_SAMP_CYCLE_NUMBER次波形 + 单次1024/2048个点(地址×4代表每次递增32位 因为Xil_Out32一次写32位)
			Xil_Out32(Share_addr + j*sample_points*4 + (AD_SAMP_CYCLE_NUMBER-ADC_Sampling_ddr)*CHANNL_MAX*sample_points*4 + i*4,(u32)dma_rx_8[j][i]);
		}
	}
	ADC_Sampling_ddr--;
